...
export default class KickCommand {
	...
	if (!member || member.id == interaction.user.id)
	    return interaction.reply(`O membro mencionado não pode ser expulso.`);
	...
	if (!author?.permissions.has(PermissionFlagsBits.KickMembers))
	    return interaction.reply(`Você não tem permissão para expulsar um membro.`);
	else if (!interaction.guild?.members.me?.permissions.has(PermissionFlagsBits.KickMembers))
	    return interaction.reply(`Eu não tenho permissão de expulsar um membro.`);
